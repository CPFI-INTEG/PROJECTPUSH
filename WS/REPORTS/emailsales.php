...
if($usertype=='Distributor'){
 $sqlModal="select * from dbo.Vw_PUSHSALES where  TS like (select   CONVERT(VARCHAR(10), SYSDATETIME(), 103)) and JobberID='".$JobberId."' AND usertype='".$usertype."'  AND JobberName='".$Distributor."'";
 }elseif ($usertype=='Admin') {
  $sqlModal="select * from dbo.Vw_PUSHSALES where  TS like (select  CONVERT(VARCHAR(10), SYSDATETIME(), 103))";
}
      //echo $sqlModal;
      ...
      while($row=sqlsrv_fetch_array($stmt,SQLSRV_FETCH_ASSOC)){
       ...
       $objPHPExcel->getActiveSheet()->setCellValueExplicit ('A'.$x, $row['TS'],PHPExcel_Cell_DataType::TYPE_STRING);  
        ...
        $objPHPExcel->getActiveSheet()->setCellValue ('V'.$x,'=M'.$x.'*U'.$x.'');
